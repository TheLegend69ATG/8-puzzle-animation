...
if __name__ == '__main__':
    import time
    mode=sys.argv[1]
    initial=tuple(map(lambda x:int(x),tuple(sys.argv[2])))
    goal=tuple(map(lambda x:int(x),tuple(sys.argv[3])))
    # problem = EightPuzzleProblem(initial=(3, 1, 2, 6, 0, 8, 7, 5, 4), goal=(0, 1, 2, 3, 4, 5, 6, 7, 8))
    if not(len(initial)==9 and len(goal)==9):
    ...
    if mode=='DLS':
        ...
        try:
            ...
            print('No solution !!!! Please try another depth limit')
            sys.exit()
        list_arg=['./animation.py',str(result.solution())]
        sys.argv=list_arg
        game.main()
    elif mode=='IDS':
